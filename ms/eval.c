...
#include "../include/pain_log.h"

void Release_IfMString(Value* v) {
    if (v->type == ValueType_String) MStringRelease(v->u.mstring);
}
void Retain_IfMString(Value* v) {
    if (v->type == ValueType_String) MStringRetain(v->u.mstring);
}
...
Variable* eval_getVariable(Interpreter* inter, Environment* localenv, char* identifier) {
...
//paramlist 形参
void ParamListAssign(Interpreter* inter, Environment* outerEnv, Environment* innerEnv,
                     ParamList* paramlist, Function_t* func) {
...
Value execute_Expression_Primary(Interpreter* inter, Environment* local, Expression* expression) {
    ...
    case IDENTIFIER_TYPE: {
        ...
        if (variable) {
            ...
            //
            printf("can not find var %s\n", primaryExpression->u.identifier);
...
#define allInteger(leftv, rightv) \
    ((leftv).type == ValueType_Integer && (rightv).type == ValueType_Integer)

#define orString(leftv, rightv) \
    ((leftv).type == ValueType_String || (rightv).type == ValueType_String)
...
Value execute_AssignExpresion(Interpreter* inter, Environment* local, Expression* expression) {
    Value v;
...
StatementResult execute_ForStatement(Interpreter* inter, Environment* local, Statement* statement) {
    StatementResult result;
    result.result = StatementResultType_Normal;
    For_Statement* forstatement = statement->u.f;
    if (forstatement->before) execute_Expression(inter, local, forstatement->before);
    while (1) {
        // printf("for :%d\n", __LINE__);
        if (forstatement->condition) {
            Value v = execute_Expression(inter, local, forstatement->condition);
            if (v.type == ValueType_Integer && v.u.i == 0) break;
            if (v.type == INVALID_TYPE) break;
        }
        // printf("stp:%p\n", forstatement->statementlist);
        StatementResult exeresult =
                execute_StatementList(inter, local, forstatement->statementlist);
        if (exeresult.result == StatementResultType_Break) break;
        if (exeresult.result == StatementResultType_Rtn) return exeresult;
        if (forstatement->after) execute_Expression(inter, local, forstatement->after);
    }
    return result;
}

StatementResult execute_IFStatement(Interpreter* inter, Environment* local, Statement* statement) {
    StatementResult result;
    ...
    if (v.type == ValueType_Integer) {
        if (v.u.i != 0) execute_StatementList(inter, local, if_Statement->statementlist);
    } else if (v.type == ValueType_Double) {
        if (v.u.d != 0) execute_StatementList(inter, local, if_Statement->statementlist);
    } else if (v.type == ValueType_Native) {
        if (v.u.p != NULL) execute_StatementList(inter, local, if_Statement->statementlist);
    }
    result.result = StatementResultType_Normal;
}

StatementResult execute_Statement(Interpreter* inter, Environment* local, Statement* statement) {
    StatementResult result;
...
StatementResult execute_StatementList(Interpreter* inter, Environment* local,
    ...
    for (pos = statementlist; pos; pos = pos->next) {
        ...
        if (result.result != StatementResultType_Normal) break;
    }
    return result;
}
...
void mprint(Value* v) {
    ...
    case ValueType_INVALID:
        printf("[INVALID] %s", v->u.mstring->str);
        break;
    case ValueType_Native:
        printf("[Native] %s", v->u.mstring->str);
        break;
    default:
        printf("[ERROR TYPE] %s", v->u.mstring->str);
        abort();
        break;
    }
}
...
void RegistNativeProc(Interpreter* inter, NativerFuncProc* nativefunc, char* identifier,
                      int argsnum) {
